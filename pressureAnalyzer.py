...
class PressureAnalyzer:
    def __init__(self, time_data, pressure_data, sample_rate=100):
        ...
        self.second_derivative = None  # Second derivative of pressure signal
        self.third_derivative = None  # Third derivative of pressure signal
    ...
    def get_diastolic_width(self, percent=50, time_distance=1):
        """
        Calculate the Diastolic Width (DW##) for each cycle, defined as t(x = ##) - t(sys).
        t(x = ##) is the time where the signal falls to ##% of the amplitude on the falling limb.

        Parameters:
        - percent: Percentage of amplitude (e.g., 50 for DW50, range 0-100)
        ...
        while i < n_systolic and j < n_onset:
            ...
            if abs(sys_time - onset_time) < time_distance and sys_time > onset_time:
                ...
                # Find the first point where pressure drops to or below target_value
                crossing_idx = np.where(segment_pressure <= target_value)[0]
                if len(crossing_idx) > 0:
                    t_x = segment_time[crossing_idx[0]]
                    dw = t_x - sys_time
                    if dw > 0:  # Ensure positive width
                        dw_times.append(dw)
                i += 1
                j += 1
    ...
    def plot_results_all(self, fig=None, ax=None):
        ...
        ax1.plot(self.time, self.pressure, 'b-', label='Pressure Waveform')
...
if __name__ == "__main__":
    try:
        t = np.load(
            "storage/2025-02-18 14_21_53 น้ำ 8000 120 (320, 320)/ippg/time_ppg.npy")
        pressure = np.load(
            "storage/2025-02-18 14_21_53 น้ำ 8000 120 (320, 320)/ippg/filtered_ppg.npy")
    ...
    parser.add_argument("--sample_rate", type=int,
                        default=120, help="Sampling rate in Hz")
    ...
    analyzer.find_pressure_features(
        min_distance=0.5, height_percentile=75, diastolic_prominence=0.001, onset_search_range=0.15)
