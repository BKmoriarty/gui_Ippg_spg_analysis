...
class RecordingVideoPulse():
    def __init__(self, port_name='COM10', exposure_time=2200, name: str = 'unknown', fps=30, size=(600, 600), length=20):
            ...
            if self.ser.in_waiting > 0 and not self.camera.stop:
                ...
                if line.startswith("PULSE,"):
                    ...
                    if len(parts) == 5:
                        ...
                        serial_data.append([pd.Timestamp.now().time(), datetime.timestamp(
                            datetime.now()), value, status, checksum, timestamp, 0])

        if len(serial_data) > 0:
            frame = self.fps * self.length
            current_frame = len(serial_data)
            if not current_frame == frame:
                if current_frame > frame:
                    serial_data = serial_data[:frame]

                if current_frame < frame:
                    # use moving avg 10 to predict
                    moving_avg = np.mean([row[2] for row in serial_data[-10:]])
                    while len(serial_data) < frame:
                        serial_data.append([pd.Timestamp.now().time(), datetime.timestamp(
                            datetime.now()), moving_avg, serial_data[-1][3], serial_data[-1][4], serial_data[-1][5], serial_data[-1][6]])
            ...
            print(f"Serial data length: {len(serial_data)}")
    ...
    def run(self, review=False):
        if review:
            self.camera.display_images(True)

        plt.close('all')

        os.makedirs(
            f'storage/{self.current_time} {self.name} {self.ExposureTime} {self.fps} {self.frame}', exist_ok=True)

        thread1 = threading.Thread(target=self.camera.start, name='camera')
        thread2 = threading.Thread(target=self.save_serial_sync, name='serial')
...
if __name__ == '__main__':
    ...
    name = 'test'  # input("Enter your name: ")

    for i in range(1):
        for j in range(1):
            # exposure_time = [6000, 6500, 7000, 7500, 8000][i]
            exposure_time = 8000
            main = RecordingVideoPulse(
                port, exposure_time, name, fps, size_cam, length)
