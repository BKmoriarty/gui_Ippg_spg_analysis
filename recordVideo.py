...
class RecordVideo(QThread):
    ...
    def acquire_images(self, cam, nodemap):
        ...
        try:
            ...
            cam.LineSource.SetValue(
                PySpin.LineSource_ExposureActive)  # Start pulses
            ...
            while (self.continue_recording):
                try:
                    ...
                    if image_result.IsIncomplete():
                        ...
                        cropped_image = image_data[:self.size_ippg,
                                                   :self.size_ippg]
                        ...
                        if len(frames_buffer) >= self.fps * 5:
                            def process_spectrum():
                                freqs, spectrum, max_freq, snr = self.calculate_spectrum(
                                    frames_buffer)
                                self.update_plot_signal.emit(
                                    frames_buffer, [freqs, spectrum, max_freq, snr])
                                frames_buffer.clear()

                            spectrum_thread = threading.Thread(
                                target=process_spectrum)
    ...
    def calculate_snr(self, freqs, fft_result, n):
        signal_freq_range = (0.83, 4)
        noise_freq_ranges = [
            (0, signal_freq_range[0]), (signal_freq_range[1], 15)]

        power = np.abs(np.array(fft_result))**2 / n
        signal_power = np.sum(
            power[(freqs >= signal_freq_range[0]) & (freqs <= signal_freq_range[1])])
        noise_power = np.sum(
            power[(freqs >= noise_freq_ranges[0][0]) & (freqs <= min(noise_freq_ranges[0][1], 10))])
        noise_power += np.sum(power[(freqs >= noise_freq_ranges[1][0])
                                    & (freqs <= min(noise_freq_ranges[1][1], 10))])
        epsilon = 1e-10
    ...
    def calculate_spectrum(self, signal):
        ...
        pos_mask = freqs >= 0
        freqs_old = freqs[pos_mask]
        magnitude = np.abs(fft_result)[pos_mask]

        signal_freq_range = (0.83, 4)
        filtered_ppg, filtered_signal_fft1 = self.filter_signal(
            signal, self.fps,  signal_freq_range[0], signal_freq_range[1])
        fps = self.fps
        n = len(filtered_ppg)
        fft_result = np.fft.fft(filtered_ppg)
        freqs = np.fft.fftfreq(n, d=1/fps)
        snr = self.calculate_snr(fft_result, freqs, n)
        max_filtered_ppg = self.find_one_peaks(filtered_signal_fft1[1])
        heart_rate_filtered_ppg = filtered_signal_fft1[0][max_filtered_ppg]

        return freqs_old, magnitude, heart_rate_filtered_ppg, snr
    ...
    def display_images(self, show_windows=False):
